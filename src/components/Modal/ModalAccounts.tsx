...
import {DeleteAccount, VerifyPin} from '../../nfc/wallet';
...
interface Props {
  ...
  setAccountDetails: SetStateFunction<IAccount>;
  setModalVisibleImportAccount: SetStateFunction<boolean>;
}
...
const ModalAccounts = ({
  ...
  const indexCurrent = accounts.findIndex(
    item => item.address === account.address,
  );

  const _setAccountSelect = (index: number) => {
    dispatch(setAccountSelect(index));
    setModalVisible(false);
    setModalVisibleMenu(false);
  };

  const _openModalPin = (index: number) => {
    setIndexDelete(index);
    setModalVisiblePin(true);
  };

  const _deleteAccount = async () => {
    const isVerify = await VerifyPin(pin);
    if (isVerify) {
      const isDelete = await DeleteAccount(indexDelete);
      if (isDelete) {
        const response = await axiosInstance(
          `card/deleteAccount/${indexDelete}`,
        );
        dispatch(setAccounts(response.data.data.listAddress));
        if (indexDelete === indexCurrent) {
  ...
  const openAccountDetails = (account: IAccount) => {
  ...
  const openImportAccount = () => {
    setModalVisible(false);
    setModalVisibleImportAccount(true);
  };
            ...
            <ScrollView
              ...
              {accounts.map((item, index) => {
                return (
                  <TouchableOpacity
                    key={index}
                    onPress={() => _setAccountSelect(index)}>
                    <View style={styles.modalContenItem}>
                      ...
                      {item.type === 'Import' && (
                        <TouchableOpacity onPress={() => _openModalPin(index)}>
